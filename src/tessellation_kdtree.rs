...
#[wasm_bindgen]
impl TessellationKdTree {
    #[wasm_bindgen(constructor)]
    pub fn new(bounds: BoundingBox) -> TessellationKdTree {
        TessellationKdTree {
            bounds,
            generators: Vec::new(),
            cells: Vec::new(),
    ...
    pub fn set_generator(&mut self, index: usize, x: f64, y: f64, z: f64) {
        ...
        if offset + 2 < self.generators.len() {
            ...
            // Rebuild tree for simplicity as KdTree is static. 
            // For dynamic updates, a different structure or partial rebuild would be needed,
            // but Voronoi usually implies a full recalc anyway.
            self.tree.build(&self.generators);
        }
    }
    ...
    pub fn calculate(&mut self) {
        ...
        self.cells = (0..count).into_par_iter().map(|i| {
            ...
            tree.query([gx, gy, gz], current_max_dist_sq, generators, &mut |j, limit_sq| {
                ...
                // Note: The tree query already prunes based on 4 * max_dist_sq,
                // but we double check here or just proceed to clip.
                // The clip function is expensive, so a distance check is good.
